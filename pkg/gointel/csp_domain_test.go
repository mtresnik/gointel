...
func (M *mapColoringConstraint) ReduceDomain(variable string, assignment map[string]string, domain []string) []string {
	return domain
}
...
func TestCSPDomain_FindAllSolutions(t *testing.T) {
	...
	pointers := make([]*Constraint[string, string], len(constraints))
	for index, constraint := range constraints {
		pointers[index] = &constraint
	}
	csp.AddAllConstraints(pointers...)
